...
int negate(int x);
int isTmax(int x) {
  x = ~x; //~Tmax(011...1) = Tmin(100...0)
  return equal(negate(x), x) & !!(x); //排除0
}
/* 
...
int isAsciiDigit(int x) {
  return !(x & 0xfffffff8 ^ 0x30) | !(x & 0xfffffffe ^ 0x38);
}
/* 
...
int isLessOrEqual(int x, int y) {
  // x<y -> x+(-y)<0
  // 注意: 1.溢出 2.相等
  int flagx = x>>31, flagy = y>>31;
  return ((flagx ^ flagy)&(flagx&1)) | (!(flagx ^ flagy)) & ((x+negate(y))>>31) | !(x^y);
}
//4
/* 
...
unsigned floatScale2(unsigned uf) {
  ...
  if(e == 0) return s | frac<<1; //非规格化数
  if(e == 255) return uf; // NaN / 无穷
  else e++;
  if(e == 255) frac=0; // 无穷
...
unsigned floatPower2(int x) {
  if (x > 127) return 0x7f800000; //超过浮点数指数上限
